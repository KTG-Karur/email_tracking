...
if ($type == 'addDownloadToll') {
    ...
    $insertResult = pg_query_params(
        $con,
        "INSERT INTO tbl_download_toll (isp_type, group_type, list_id, sub_seg_id, seg_id, fbl_type, offer_suppression_id, esp_bounce_type, unique_type, dnd_type, data_file_name, suppression_file_name, status, create_by, c_date) 
    ...
    $updateDownloadToll = pg_query_params(
        $con,
        "UPDATE tbl_download_toll 
         SET data_file_count=$1, suppression_file_count=$2, suppression_file_ids=$3, execution_time=$4, offer_suppression_count=$5, fbl_suppression_count=$6, esp_suppression_count=$7, bounce_suppression_count=$8, complaint_suppression_count=$9
         WHERE download_id=$10",
        [$dataFileFilterCount, $suppFileFilterCount, $suppFileFilterIds, $execution_time, $offerSuppCount, $fblSuppCount, $espSuppCount, $bounceSuppCount, $complaintSuppCount, $download_id]
    );

    if ($updateDownloadToll) {
        $data = ['status' => 'true', 'id' => $download_id, 'download' => $suppFileFilterIds, 'filename' => $suppression_file_name, 'unique' => $_REQUEST['unique_type']];
    } else {
